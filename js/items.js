...
export let shoppingCart = document.querySelector("header .main-header .shopping-cart");
...
function checkItemsCart(selectedItem, quantity = 1) {
  let exist = false;
...
export function quickView(selectedItem) {
  ...
  fetch("dataProducts.json")
    ...
    .then((data) => {
      data.forEach((item) => {
        if(selectedItem.querySelector(".title-item").textContent.trim() === item.title) {
  ...
  if (selectedItem.querySelector(".discount") !== null) {
    ...
    let numPrice = price.innerText.split('').filter((ch)=> ch !== "$" && ch !== "." && ch !== ",").join('');
    let numDiscount = discount.innerText.split('').filter((ch)=> ch !== "$" && ch !== "." && ch !== ",").join('');
    discountPrecent.innerText = `Save ${100 - ((+numPrice / +numDiscount) * 100)}%`;
  ...
  inputBtn.addEventListener("click", () => {
    if(+inputQuantity.value > 0){
      numItemsInCart +=  +inputQuantity.value;
      totalItems.innerText = numItemsInCart;
      checkItemsCart(selectedItem, +inputQuantity.value);
...
function popupAddItem(selectedItem) {
  ...
  title.classList.add("title");
  title.innerText = selectedItem.querySelector(".title-item").textContent.trim();

  let price = document.createElement("p");
  price.classList.add("price");
  price.innerText = selectedItem.querySelector(".actual-price").textContent.trim();
  ...
  let priceVal = price.innerText.split('').filter((ch)=> ch !== "$" && ch !== "." && ch !== ",").join('');
